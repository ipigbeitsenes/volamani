...
<div class="mb-4">
    ...
    <p class="text-muted mb-0 small">Review physical-order returns: approve, decline, then confirm receipt to refund the buyer.</p>
</div>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                @if($r->canApprove())
                                    <form method="POST" action="{{ route('support.returns.approve', $r) }}" class="d-inline">@csrf<button class="btn btn-sm btn-success">Approve</button></form>
                                    <button class="btn btn-sm btn-outline-danger" data-bs-toggle="collapse" data-bs-target="#srej-{{ $r->id }}">Reject</button>
                                @elseif($r->canConfirmReceived())
                                    <form method="POST" action="{{ route('support.returns.confirm', $r) }}" class="d-inline" onsubmit="return confirm('Refund the buyer and restock?');">@csrf<button class="btn btn-sm btn-primary">Confirm &amp; refund</button></form>
                                @else
                        ...
                        @if($r->canReject())
                        <tr class="collapse" id="srej-{{ $r->id }}">
                            <td colspan="5" class="bg-light">
                                <form method="POST" action="{{ route('support.returns.reject', $r) }}" class="row g-2 align-items-end">
                                    @csrf
